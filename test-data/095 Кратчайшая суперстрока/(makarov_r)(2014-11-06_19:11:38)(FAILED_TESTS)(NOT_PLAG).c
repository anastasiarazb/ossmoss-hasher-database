...
int main(int argc, const char * argv[]) {
    unsigned long i, j, k, m, max = 0, index1=0, index2=0, super = 0;
    int n, switcher = 0;
    scanf("%d\n", &n);
    char strings[5][100];
    for(i = 0; i < n; i++){
    ...
    for(m = 0; m<n; m++){
        ...
        if(max>0){
            for(i = strlen(strings[index1])-1; i<strlen(strings[index1])+strlen(strings[index2])-overlap(strings[index1], strings[index2])-6; i++)
