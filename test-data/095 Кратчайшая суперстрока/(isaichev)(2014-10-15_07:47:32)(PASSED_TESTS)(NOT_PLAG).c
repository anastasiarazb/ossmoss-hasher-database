...
int Permut (int n, unsigned char **overlap, int sum) {
    int i, minsum = sum;
    for (i = 0; i<n; ++i)
        minsum = PermutRec (n, (1<<n)-(1+(1<<i)), overlap, i, sum, minsum);
    return minsum;
}
...
int PermutRec (int n, int Q, unsigned char **overlap, int prevstep, int sum, int minsum) {
    ...
    if (Q)
        for (i = 0; i<n; ++i)
            if ((Q>>i)&1)
                minsum = PermutRec (n, Q-(1<<i), overlap, i, sum-overlap[prevstep][i], (sum-overlap[prevstep][i]<minsum)? sum-overlap[prevstep][i]: minsum);
