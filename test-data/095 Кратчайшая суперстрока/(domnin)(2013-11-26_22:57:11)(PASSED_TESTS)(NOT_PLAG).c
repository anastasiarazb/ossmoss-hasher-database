...
int main(){
	...
	for (i = 1; i < n; i++){
		Q[i] = 10;
		Permut(Q, alllen - compare[0][i], 1, compare[0][i]);
		Q[i] = i;
	}
	printf("%d", result);
...
void Permut(int* Q, int res, int count, int min){
	int i;
	if (count == n){
		...
		if (Q[i] != 10){
			Q[i] = 10;
			if ((i != count)&&(min>compare[count][i])) Permut(Q, res - compare[count][i], count + 1, compare[count][i]);
			else Permut(Q, res - compare[count][i], count + 1, min);
			Q[i] = i;
		}
	}
}