...
{
	int n, i, j, k, f=0, StrLen=0, Max=0, strmax, stlmax, Min;
	scanf("%d", &n);
	int Matrix[n][n], M[n];	
	char strs[n][1000];
	for (i = 0; i < n; i++)
	...
	for (i=0;i<n;i++){     	
		for (j=0;j<n;j++){
        	...
        	while(f<n){
        		Matrix[f][stlmax]=-1;/*Делаем строчку и столбец, в которых был найден максимальный на данной итерации элемент "недоступными"*/
        		Matrix[strmax][f]=-1;
			f++;        	
		}
