...
int suffix(const char* string, int len, const char* string2, int len2)
{
    vector_int res = vector_int_create();
    ...
    if (res.array[0] < len) return 0;

    int tmp = len2 + len - res.array[0] - 1;
    res.delete(&res);
    return tmp;
}

int find_cover(vector_int* vec, char* string, char* string2)
{
    int len1 = strlen(string);
    int len2 = strlen(string2);
    return suffix(string, len1, string2, len2);
}
...
int main(int size, char** argv)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        {
            if (i == j) continue;
            array[i][j] = find_cover(&vec,strings[j],strings[i]);
        }
    }
    permut(n);
    ...
    printf("%i %i %i",max_len, maximum, max_len - maximum);
    vec.delete(&vec);
    return 0;
}