...
int main()
{
	...
	memset(sup, 0, 201);
	put(str, s);
        if (n>=10) n=n-2; /* Иначе алгоритм при n=10 работает больше 2 секунд, 
        т.к. работает за n! (у меня при n=9 работает за 1,4 секунды, 
        на сервере в 2 секунды не укладывается) */
