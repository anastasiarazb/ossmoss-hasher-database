...
void allSets(int **arr, int res, int ind, int step, int n, int taken, int without)
{
    if (step == n - 1){
    ...
    if(ind == without){
        allSets(arr, res, ind + 1, step, n, taken, without);
        return;
    }
    int i;
    for(i = 0; i < n; i++){
        if (i != ind && (taken&(1<<i)) == 0){
            allSets(arr, res + arr[ind][i], ind + 1, step + 1, n, taken|(1<<i), without);
        }
    }
}
