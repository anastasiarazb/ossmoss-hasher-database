...
int main(int argc, const char * argv[]) {
    int i, j, k, m, max = 0, index1=0, index2=0, super = 0, len;
    int n;
    scanf("%d\n", &n);
    char strings[n][100];
    for(i = 0; i < n; i++){
    ...
    for(m = 0; m<n; m++){
        ...
        if(max>0){
            len = (strlen(strings[index1]));
            for (i = len-1; i<(strlen(strings[index1])+strlen(strings[index2])-overlap(strings[index1], strings[index2])); i++)
