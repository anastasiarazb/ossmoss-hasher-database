...
void creating(char str[][100], const int n, int counter, char super[], int* length)
{
    ...
    for (i = 0; i < (int)strlen(str[counter]); i++) {
        ...
        int flag = 1;
        for (j = 0, k = 0; (j < (int)strlen(str[counter]) - i) && (k < strlen(super)); j++, k++) {
            if (str[counter][i + j] != super[k]) flag = 0;
        }
        ...
        flag = 1;
        for (j = 0, k = strlen(super) - strlen(str[counter]) + i;
            (j < (int)strlen(str[counter]) - i) && ((k + j) < strlen(super)); j++, k++) {
            if (str[counter][j] != super[k]) flag = 0;
        }
