...
int ad_Dlina(char *a, char *b)
{
  ...
  t=strnlen(a,max);
  if(t>strnlen(b,max))  t=strnlen(b,max);
  for(i=strnlen(a,max)-1;i>=0 && ( strnlen(a,max)-i<=strnlen(b,max) );i--){
      flag=1;
      for(j=0; j<strnlen(b,max) && j<strnlen(a,max)-i;j++)  {
          if(a[i+j]!=b[j])  {
              flag=0;
              break;
          }
      }
      if(flag==1) rezult=strnlen(a,max)-i;
  }
  return rezult;
}
