...
int main() 
{
	...
	printf("len==%d,", sum_len+str_count);
	global_dest_buf=malloc((sum_len+str_count)*sizeof(char));
	permut(inds, str_count, str_count, new_arr, new_arr_size);
	printf("%d\n", min_len);
	for (i=0; i<str_count; i++) {
		free (str_arr[i]);
	}
	free (global_dest_buf);
	free (str_arr);
	return 0;
}
...
int permut(int *arr, int arr_size, int m, int *p, int p_size)
{
	...
	int res_len;
	if (m==0) {
		res_len=str_plus(p, p_size, global_dest_buf);
...
int str_plus(int *cur_permut, int permut_size, char *dest_buf)
{
	...
	if (permut_size==1) {
		...
		strcpy(dest_buf, head_str);
                dest_ptr=dest_buf+strlen(dest_buf);
                src_ptr=tail_ptr+suf_len;
